...
src/prompt-library/
  ...
  packs/
    ...
    master.yml
  release.yml                       Version stamp updated by scripts/update_version.py
```
...
archive_output: docs/prompt-libraries/v1.1/03_nhs_healthcare_reflection_library_v1_1.md
...
2. If you changed codes, titles, descriptions, order, aliases or mode, update
   `tool-metadata.json` to match.
3. Rebuild and verify everything affected by the change:
   ```bash
   python scripts/build_all.py --ci
   ```

For a public version bump, run the version updater first:
...
4. Add a worked example source record in `src/examples/items/`.
5. Rebuild with `python scripts/build_all.py --ci`. The manifest, launcher, router, examples and site data update automatically.

## Version updates

Do not manually chase version strings through prompt files. Use:

```bash
python scripts/update_version.py <version> --date YYYY-MM-DD
```

This updates `release.yml`, pack archive paths, visible prompt headings, manifest stamps, audit-pack stamps and `CHANGELOG.md`. Then run `python scripts/build_all.py --ci` to regenerate the public prompt libraries, site data and changelog page.
