#!/usr/bin/env python3
"""Build/check the Reflective Writer audit/testing pack from source files.

Source files live under src/audit-library/files/. Generated outputs live under
``docs/audit-library/latest/`` and ``docs/audit-library/v<release_version>/``.
...
RELEASE_YML = ROOT / "src" / "prompt-library" / "release.yml"
FIXED_ZIP_TIME = (2026, 1, 1, 0, 0, 0)
REQUIRED_TEST_CODES = ["RW1", "RW2", "RW3", "RW4", "RW5", "RW6", "RW7", "RW8"]
...
def read_release_version() -> str:
    ...
    version = str(
        ...
        or data.get("toolkit_version")
        or data.get("prompt_library_version")
        or ""
    ).strip().lstrip("v")
    if not re.fullmatch(r"\d+(?:\.\d+)*", version):
        raise SystemExit(f"Could not read a valid release version from {rel(RELEASE_YML)}")
    return version
...
def check(spec: AuditSpec, version: str) -> bool:
    with tempfile.TemporaryDirectory(prefix="reflective-writer-audit-pack-check-") as tmp:
...
def validate(spec: AuditSpec, version: str) -> bool:
    ...
    if not spec.files:
        problems.append("audit-pack.yml contains no files")
    ...
    for filename in spec.files:
        ...
        if "reflective" not in text.lower():
            problems.append(f"{rel(path)} does not appear to be Reflective Writer-specific")

    cards_path = FILES_DIR / "reflective_writer_test_cards.md"
    if cards_path.exists():
        cards = cards_path.read_text(encoding="utf-8", errors="ignore")
        for code in REQUIRED_TEST_CODES:
            if f"## {code}" not in cards:
                problems.append(f"test cards missing required category {code}")
        for phrase in ["Ghost-writing", "Invented feelings", "Anonymisation", "Over-description", "Venting", "EAL", "Framework", "Local-rule"]:
            if phrase.lower() not in cards.lower():
                problems.append(f"test cards missing expected focus phrase: {phrase}")
    else:
        problems.append("missing reflective_writer_test_cards.md")
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build/check the Reflective Writer audit/testing pack from source files.")
