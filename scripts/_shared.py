"""Shared building blocks for the Reflective Report Writing Tutor libraries.

These blocks are reused across every mini library and the master library so the
core "support reflection, do not invent it" design stays identical everywhere.
"""
...
# ---------------------------------------------------------------------------
# Activation instruction (top of every file)
# ---------------------------------------------------------------------------

def activation(library_name: str) -> str:
    return f"""# READ THIS FIRST — ACTIVATION INSTRUCTION
...
# ---------------------------------------------------------------------------
# Global rules (the heart of the toolkit; shared by every tool)
# ---------------------------------------------------------------------------

GLOBAL_RULES = """<!-- FILE: 01-global-rules.md -->
---
id: global-rules
...
# ---------------------------------------------------------------------------
# Markdown output rules (shared)
# ---------------------------------------------------------------------------

MARKDOWN_RULES = """<!-- FILE: 02-markdown-output-rules.md -->
---
...
def manifest(library_title: str, code_prefix: str, tools, public_path: str, archive_path: str) -> str:
    rows = []
    for t in tools:
        rows.append(
            f"| {t['menu']} | {t['code']} | {t['id']} | {t['title']} | {t['use_when']} |"
        )
    table = "\n".join(rows)
    return f"""<!-- FILE: 00-manifest.md -->
---
id: manifest
title: {library_title}
type: manifest
...
**Release stamp:** Reflective Writer toolkit v{VERSION} / Prompt-library suite v{VERSION}
**This file:** {library_title} v{VERSION}
...
def launcher(library_title: str, purpose_line: str, menu_lines, footer_lines) -> str:
    menu = "\n".join(menu_lines)
    footer = "\n\n".join(footer_lines)
    return f"""<!-- FILE: 03-launcher.md -->
---
id: launcher
title: {library_title} Launcher
type: launcher
...
def router(library_title: str, mapping_lines, extra_notes: str = "") -> str:
    mapping = "\n".join(mapping_lines)
    return f"""<!-- FILE: 04-router.md -->
---
id: router
title: Router
type: router
